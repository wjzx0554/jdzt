...
import contextlib, os, queue, sys, threading, traceback
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.log_queue=queue.Queue(); self.input_dir=tk.StringVar(); self.work_dir=tk.StringVar(); self.output_dir=tk.StringVar(); self.config_file=tk.StringVar(); self.systemdb_file=tk.StringVar(); self.mapping_file=tk.StringVar(); self.allow_unconfirmed=tk.BooleanVar(value=False)
    ...
    def build(self):
        ...
        self.row(frm,0,'原始多年账套目录',self.input_dir,'dir'); self.row(frm,1,'加工区/映射输出目录',self.work_dir,'dir'); self.row(frm,2,'处理后账套副本目录',self.output_dir,'dir'); self.row(frm,3,'配置文件 config.json',self.config_file,'save'); self.row(frm,4,'System.mda 工作组文件',self.systemdb_file,'systemdb'); self.row(frm,5,'科目旧新映射 CSV',self.mapping_file,'file')
        ttk.Label(frm,text='流程：生成配置 → 读取账套生成映射草稿 → 人工复核 CSV → 试运行 → 正式生成账套副本',foreground='#555').grid(row=6,column=0,columnspan=3,sticky='w',pady=(8,0))
    ...
    def choose(self,var,kind):
        ...
        elif kind=='systemdb': p=filedialog.askopenfilename(filetypes=[('金蝶工作组文件','System.mda'),('Access 工作组文件','*.mda'),('所有文件','*.*')],initialfile='System.mda')
    ...
    def run_core(self,argv,done='完成。'):
        def task():
            old=sys.argv[:]; old_systemdb=os.environ.get('KIS_SYSTEMDB'); systemdb=self.systemdb_file.get().strip(); self.log_queue.put('\n>>> '+' '.join(argv)+'\n')
            try:
                if systemdb: os.environ['KIS_SYSTEMDB']=systemdb
                ...
                sys.argv=old
                if old_systemdb is None: os.environ.pop('KIS_SYSTEMDB',None)
                else: os.environ['KIS_SYSTEMDB']=old_systemdb
    ...
    def inspect(self):
        ...
        if self.systemdb_file.get().strip(): argv+=['--systemdb',self.systemdb_file.get()]
